...
import com.example.test_trianglify.trianglify.utilities.ExtendedColor;
import com.example.test_trianglify.trianglify.utilities.Point;
import com.example.test_trianglify.trianglify.utilities.ThreadLocalRandom;
import com.example.test_trianglify.trianglify.utilities.triangulator.Triangle2D;
...
public class FixedPointsColorInterface implements ColorInterface {
    ...
    @Override
    public Triangulation getColororedTriangulation(Presenter.TrianglifyGenerateListener listener) {
        if (triangulation != null) {
            List<Triangle2D> triangles = triangulation.triangleList();
            for (int i = 0; i < triangles.size(); i++) {
                Triangle2D triangle = triangles.get(i);
                triangle.setColor(getColorForPoint(triangle.getCentroid()));
                listener.onTriangulationGenerationInProgress(triangles.size(), i + 1,
                        "Generating Colour");
            }
        } else {
            Log.e(TAG, "colorizeTriangulation: Triangulation cannot be null!");
